      ...
      <div className="container mx-auto flex h-14 items-center justify-between px-4">
        ...
        <nav className="hidden gap-0.5 md:flex" aria-label="Main navigation">
          {navLinks.map(({ href, label }) => (
            <Link
              key={href}
              href={href}
              className="hover-ink rounded px-3 py-1.5 text-xs uppercase tracking-widest"
            >
              {label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-2">
          ...
          <a
            ...
            className="flex items-center gap-1.5 rounded px-3 py-1 text-xs tracking-wide transition-opacity hover:opacity-75"
